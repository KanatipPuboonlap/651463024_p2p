import socket  # โมดูลสำหรับการสื่อสารผ่านเครือข่าย
import threading  # โมดูลสำหรับการทำงานแบบ multi-threading
import json  # โมดูลสำหรับการจัดการ JSON
import sys  # โมดูลสำหรับการจัดการค่าพารามิเตอร์จาก command line
import os  # โมดูลสำหรับการจัดการระบบไฟล์
import secrets  # โมดูลสำหรับการสร้างข้อมูลสุ่มที่ปลอดภัย

class Node:
    def __init__(self, host, port):
        self.host = host  # เก็บค่า host ที่โหนดจะรัน
        self.port = port  # เก็บค่า port ที่โหนดจะรัน
        self.peers = []  # เก็บรายการ socket ของ peer ที่เชื่อมต่อ
        self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # สร้าง socket สำหรับโหนด
    ...
    def start(self):
        ...
        self.socket.listen(1)  # เริ่มฟังการเชื่อมต่อ
    ...
    def handle_client(self, client_socket):
        while True:
            try:
                ...
                if not data:
                    break
                message = json.loads(data.decode('utf-8'))  # แปลงข้อมูลจาก JSON
    ...
    def broadcast(self, message):
        ...
        for peer_socket in self.peers:
            try:
                ...
                self.peers.remove(peer_socket)  # ลบ peer ที่เชื่อมต่อไม่ได้ออกจากรายการ
...
if __name__ == "__main__":
    ...
    port = int(sys.argv[1])  # รับค่า port จาก command line
